...
def test_class2_preview_is_deterministic_and_complete(tmp_path):
    ...
    first = build(source, "CLASS2-05")
    second = build(source, "CLASS2-05")
    assert json.dumps(first, ensure_ascii=False, sort_keys=True) == json.dumps(second, ensure_ascii=False, sort_keys=True)
    expected_slugs = json.loads((ROOT.parent.parent / ".local-work" / "class2-chunks.json").read_text(encoding="utf-8"))["chunks"][4]["slugs"]
    assert first["chunkId"] == "CLASS2-05"
...
def test_preview_contains_provenance_and_does_not_claim_legal_fields():
    artifact = build(ROOT.parent.parent / ".local-work" / "unresolved-geometry-inventory.json", "CLASS2-05")
...
def test_generation_does_not_change_canonical_input():
    ...
    build(ROOT.parent.parent / ".local-work" / "unresolved-geometry-inventory.json", "CLASS2-05")
